...
y2, fields = wfdb.rdsamp('lobachevsky-university-electrocardiography-database-1.0.1/data/1', sampfrom=1400, channels=[0])
ynew = np.transpose(y2)
y = ynew.tolist()
y = y[0]
...
for i in range(N):
    y[i] = y[i] + 0.2*np.cos(120*2*np.pi*i/500)
...
fs = 500  # The example dataset was recorded at 300Hz
